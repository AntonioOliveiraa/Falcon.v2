...
void loop() {
  ...
  // motorRamp(motor, vel. init, vel. fin., mode, interval-delay);
  // moveForward(motor1, motor2, velocidade);
  // moveBackward(motor1, motor2, velocidade);
  // stopMove(motor1, motor2);

  if (mover) {
    moveForward(motor1, motor2, 90);
    delay(2000);
    mover = false;
  }
